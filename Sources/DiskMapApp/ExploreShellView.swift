...
struct ExploreCanvas: View {
    ...
    var body: some View {
        Group {
            ...
            case .sunburst:
                LayoutChartView(kind: .sunburst, tree: tree, totals: totals, currentNode: $model.currentNode, otherFraction: otherFraction)
                    .onChange(of: model.currentNode) { _, v in model.selectedNode = v }
            case .flame:
                LayoutChartView(kind: .flame, tree: tree, totals: totals, currentNode: $model.currentNode, otherFraction: otherFraction)
                    .onChange(of: model.currentNode) { _, v in model.selectedNode = v }
            case .bubbles:
                LayoutChartView(kind: .bubbles, tree: tree, totals: totals, currentNode: $model.currentNode, otherFraction: otherFraction)
                    .onChange(of: model.currentNode) { _, v in model.selectedNode = v }
            case .mindMap:
...
struct ExploreTreemapView: View {
    ...
    private var topLevelHues: [Int32: Int] {
        var map: [Int32: Int] = [:]
    ...
    private func colorFor(id: Int32) -> Color {
        switch colorMode {
        case .folder:
            return folderColor(id: id)
        case .type:
            return typeColor(id: id)
        case .age:
            return ageColor(id: id)
        }
    }

    private func folderColor(id: Int32) -> Color {
        var cursor = id
        var top = id
        ...
        let idx = topLevelHues[top] ?? Int(id)
        let base = DiskMapTheme.folderPastels[abs(idx) % DiskMapTheme.folderPastels.count]
        // Descendants: slightly brighter/darker shade
        let depth = max(0, ancestorDepth(id) - ancestorDepth(top))
        return depth == 0 ? base : base.opacity(1.0 - min(0.35, Double(depth) * 0.08))
    }

    private func ancestorDepth(_ id: Int32) -> Int {
        var d = 0
        var c = id
        while c >= 0 {
            d += 1
    ...
    private func typeColor(id: Int32) -> Color {
    ...
    private func ageColor(id: Int32) -> Color {
